...
test('session hook injects context only inside the repository', () => {
  ...
  const inside = createSessionStartOutput(
    { cwd: '/workspace/recovery-client/src/app' },
    repoRoot,
  )
  const outside = createSessionStartOutput({ cwd: '/workspace/other' }, repoRoot)

  assert.equal(
    inside.hookSpecificOutput.hookEventName,
    'SessionStart',
  )
...
test('tokenizes quoted arguments and compound shell commands', () => {
  assert.deepEqual(
    tokenizeShell('pnpm exec "next build" && git status'),
    ['pnpm', 'exec', 'next build', '&&', 'git', 'status'],
  )
})
...
test('returns no payload for safe or missing commands', () => {
  assert.equal(
    createPreToolUseOutput({ tool_input: { command: 'pnpm build' } }),
    null,
  )
