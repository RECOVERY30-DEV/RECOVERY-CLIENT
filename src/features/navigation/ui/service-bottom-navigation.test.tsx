...
describe('서비스 하단 내비게이션', () => {
  it('주요 서비스 경로를 제공하고 현재 경로를 표시한다', () => {
    ...
    expect(screen.getByRole('link', { name: '회복안' })).toHaveAttribute(
      'href',
      '/recovery/compare',
    )
    ...
    expect(screen.getByRole('link', { name: '관리' })).toHaveClass('text-primary-blue-500')
    expect(screen.getByRole('link', { name: '홈' })).not.toHaveAttribute('aria-current')
    expect(screen.getByRole('navigation', { name: '주요 메뉴' })).toHaveClass('px-4')
